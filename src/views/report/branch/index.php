<div class="card mb-4">
    <div class="card-header">
        <h3 class="card-title">Средняя оценка по сотрудникам филиала</h3>
    </div>
    <div class="card-body border-bottom py-3">
        <div class="form-group">
            <label class="form-label" for="branch">Выберите филиал:</label>
            <select name="branch" class="form-select mb-4" id="branch">
                <?php foreach ($branches as $branch): ?>
                    <option value="<?php echo $branch['code']; ?>" <?php echo ($branch['code'] == $branch_code) ? 'selected' : ''; ?>>
        ...
        <div class="card-body border-bottom py-3">
            <div class="d-flex">
                <div class="btn-list ms-auto">
                    <a href="/report/branch/export/<?php echo $branch_code; ?>/<?php echo $month; ?>/<?php echo $year; ?>" class="btn btn-green">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            window.ApexCharts && (new ApexCharts(document.getElementById('chart'), {
                ...
                xaxis: {
                    categories: <?php echo json_encode($employeeNames); ?>,
                    labels: {
                        padding: 0,
                    },
                    tooltip: {
...
<script>
    document.getElementById('branch').addEventListener('change', function() {
        ...
        window.location.href = `/report/branch/${this.value}/${month}/${year}`;
    });

    document.getElementById('year').addEventListener('change', function() {
        const branch = document.getElementById('branch').value;
        const month = document.getElementById('month').value;
        window.location.href = `/report/branch/${branch}/${month}/${this.value}`;
    });

    document.getElementById('month').addEventListener('change', function() {
        const branch = document.getElementById('branch').value;
        const year = document.getElementById('year').value;
        window.location.href = `/report/branch/${branch}/${this.value}/${year}`;
    });
</script>